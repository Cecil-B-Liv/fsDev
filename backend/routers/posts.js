...
router.post("/:postId/comments/create",
    isAuthenticated,
    validateCreateComment,  // Add validation middleware
    createComment);   // For create a user comment to a post
...
router.get("/:userId",
    isAuthenticated,
    getUserPosts);      // Get all posts from a user
// router.get("/:groupId", isAuthenticated, getGroupPosts);    // *Get all posts from a group (maybe use getGroup from groups.js router)*
router.get("/admin",
    isAuthenticated,
    isSiteAdmin,
    adminGetPosts);    // Get all posts without obstruction for siteAdmin

/* UPDATE */
router.put("/:postId/react",
    isAuthenticated,
    reactPost);   // Add/Update reaction to a post
router.put("/:postId/update",
    isAuthenticated,
    validateUpdatePost,  // Add validation middleware
    uploadPostPicture,   // Add upload middleware
    updatePost);    // Update user post
router.put("/comments/:commentId/update",
    isAuthenticated,
    validateUpdateComment,  // Add validation middleware
    updateComment);  // Update user comment to a post

/* DELETE */
router.delete("/:postId/delete",
    isAuthenticated,
    deletePost);      // Delete user post with its comments
router.delete("comments/:commentId/delete",
    isAuthenticated,
    deleteComment);    // Delete user comment to a post
